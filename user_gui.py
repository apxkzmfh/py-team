...
def Userwindow(): # 메인화면에서 회원 클릭
    def Usersearch(): # 회원 조회 클릭
        def User_Show(event):   #treeview로 조회된 회원중 한명을 더블클릭 했을 때           
            ...
            #사진주소(이미지X)
            df_user = pd.read_csv('USER1.csv', encoding = 'UTF-8', index_col = 'PHONE')
            pic_lab = Label(user_show, text = "사진 : ", font = ("맑은 고딕", 12), fg = "#203864", bg = "white")
            pic_lab.grid(row = 5, column = 0, pady = 10)
            pic_text = Entry(user_show, width = 30)
            pic_text.insert(0, df_user.loc[getValue[2]]["PICTURE"])
            pic_text.place(x = 80, y = 250)
            pic_find = Button(user_show, text = "사진찾기", command = find)
            ...
            #사진
            pic_frame = Frame(user_show)
            pic_frame.place(x = 420, y = 10)

...
book_list=np.reshape(book_list,(int(book_list.size/6),6))
...
class Book:
    ...
    def _Isbnlist(self): #접근자
        return self.__book[:0]

    def save(self): # 변동사항 생길 때마다 저장(IF-007)
    ...
    def B_Info(self,inf): # 도서 등록 (IF-002)
...
def Bookwindow():
    ...
    def Bookadd():
        def addbook():
            b_list = np.array([])
            b_list = np.append(int(BISBN_text1.get()))
            b_list = np.append(Bname_text.get())
            b_list = np.append(Bauthor_text.get())
            b_list = np.append(Bpubli_text.get())
            b_list = np.append(Bprice_text.get())
            b_list = np.append(BURL_text.get())
            #b_list = np.append(Bpicture_text.get())
            b_list = np.append(True)
...
wall = PhotoImage(file = "C:/Users/PC_1M/Desktop/도서관리(gui)/메인 화면.png")
book = PhotoImage(file = "C:/Users/PC_1M/Desktop/도서관리(gui)/회원.png")
user = PhotoImage(file = "C:/Users/PC_1M/Desktop/도서관리(gui)/책.png")
rent = PhotoImage(file = "C:/Users/PC_1M/Desktop/도서관리(gui)/대여.png")

user_wall = PhotoImage(file = "C:/Users/PC_1M/Desktop/도서관리(gui)/기본 배경.png")
#X = PhotoImage(file = "C:/Users/PC_1M/Desktop/도서관리(gui)/X.png")
## 색 조합 봐달라고 하자...

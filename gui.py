...
df=pd.read_csv('C:/Users/PC_1M/Desktop/새 폴더 (4)/py-team/Book.csv', encoding='UTF-8')
book_list=np.array([])
...
class Book: #도서 클래스
    ...
    def save(self): # 변동사항 생길 때마다 저장
        ...
        bookdf=pd.DataFrame(self.__book, columns=book_col) # to_csv작동을 위해 데이터프레임으로 변환
        bookdf.to_csv('C:/Users/PC_1M/Desktop/새 폴더 (4)/py-team/Book.csv', encoding='UTF-8') 
    
    def B_Info(self,inf): # 도서 등록 
...
def Bookwindow():
    def Booksearch(): #수정필
        def printbook(): #도서조회 함수
            ...
            for i in range(int(sblist.size/7)):
                tview=[]
                if sblist[i,6]:
                    tview.append('대출 가능')
                else:
                    tview.append("대출 불가")
                for j in range(1,6):
                    tview.append(sblist[i,j-1])
                Btreeview.insert("","end",text="",values=tview,iid=i)
    ...
    def Bookadd(): #도서등록은 다 된거 같은데...
        ...
        def addbook():
            ...
            b_list = np.append(b_list, np.array([int(BISBN_text1.get()), Bname_text.get(), Bauthor_text.get(), Bprice_text.get(), BURL_text.get(), Bpubli_text.get()]))
            #b_list = np.append(Bpicture_text.get()) 사진은....
            B.B_Info([b_list])
            messagebox.showinfo("알림","도서 등록이 완료되었습니다.")# 팝업창
            checkbook.remove(Bname_text.get()) #중복되지 않게 중복확인을 위한 리스트 비워주기
